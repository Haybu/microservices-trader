...
public class ServiceTestConfiguration  {
	...
	public AccountService accountService() {
		...
		when(accountService.findAccount(ArgumentMatchers.eq(ACCOUNT_ID))).thenReturn(account());
		//when(accountService.findAccountByProfile(any(Accountprofile.class))).thenReturn(account());
		when(accountService.login(ArgumentMatchers.eq(USER_ID), ArgumentMatchers.eq(PASSWORD))).thenReturn(loginResponse());
		when(accountService.login(ArgumentMatchers.eq(BAD_USER_ID), ArgumentMatchers.eq(BAD_PASSWORD))).thenReturn(null);

		doNothing().when(accountService).logout(ArgumentMatchers.any(String.class));
