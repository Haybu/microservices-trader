...
public class PortfolioServiceTest {
	...
	@Mock QuoteRemoteCallService quoteService;
	...
	public void doGetPortfolio() {
		...
		when(repo.findByAccountId(ServiceTestConfiguration.ACCOUNT_ID)).thenReturn(ServiceTestConfiguration
				.orders());
		//when(quoteService.getUri()).thenReturn(uri);
		when(quoteService.getQuotes(ServiceTestConfiguration.quote().getSymbol())).thenReturn(Collections.singletonList(ServiceTestConfiguration
				.quote()));
	...
	public void doSaveOrder() {
		...
		double amount = ServiceTestConfiguration.order().getQuantity()*ServiceTestConfiguration
				.order().getPrice().doubleValue()+ServiceTestConfiguration.order().getOrderFee().doubleValue();
		ResponseEntity<Double> response = new ResponseEntity<Double>(100d, HttpStatus.OK);
		
		
		//when(accountService.getUri()).thenReturn(uri);
		when(restTemplate.getForEntity("http://" + service.accountsService +"/accounts/{userid}/decreaseBalance/{amount}", Double.class, ServiceTestConfiguration
				.order().getAccountId(), amount )).thenReturn(response);
	...
	public void doSaveOrderNullOrderFee() {
		...
		//when(accountService.getUri()).thenReturn(uri);
		when(restTemplate.getForEntity((String)ArgumentMatchers.any(), ArgumentMatchers.eq(Double.class), ArgumentMatchers
				.any(), ArgumentMatchers.any())).thenReturn(response);
		when(repo.save(ArgumentMatchers.isA(Order.class))).thenReturn(returnOrder);
	...
	public void doSaveOrderSellOrder() {
		...
		returnOrder.setOrderId(1);
		double amount = ServiceTestConfiguration.sellOrder().getQuantity()*ServiceTestConfiguration
				.sellOrder().getPrice().doubleValue()-ServiceTestConfiguration.sellOrder().getOrderFee().doubleValue();
		ResponseEntity<Double> response = new ResponseEntity<Double>(100d, HttpStatus.OK);
		
		
		//when(accountService.getUri()).thenReturn(uri);
		when(restTemplate.getForEntity("http://" + service.accountsService +"/accounts/{userid}/increaseBalance/{amount}", Double.class, ServiceTestConfiguration
				.sellOrder().getAccountId(), amount )).thenReturn(response);
